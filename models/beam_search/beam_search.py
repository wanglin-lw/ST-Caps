...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, object_feats: utils.TensorOrSequence, geo_weights: utils.TensorOrSequence, sen_weights: utils.TensorOrSequence, ocr_list: utils.TensorOrSequence, ocr_vec: utils.TensorOrSequence, shot_mask: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, object_feats, geo_weights, sen_weights, ocr_list, ocr_vec, shot_mask, outputs = self.iter(t, visual, object_feats, geo_weights, sen_weights, ocr_list, ocr_vec, shot_mask,outputs, return_probs, **kwargs)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, object_feats: utils.TensorOrSequence, geo_weights: utils.TensorOrSequence, sen_weights: utils.TensorOrSequence, ocr_list: utils.TensorOrSequence, ocr_vec: utils.TensorOrSequence, shot_mask: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words, visual, object_feats, geo_weights, sen_weights, None, ocr_list, ocr_vec, shot_mask, mode='feedback', **kwargs)
        ...
        object_feats = self._expand_visual(object_feats, cur_beam_size, selected_beam)
        geo_weights = self._expand_visual(geo_weights, cur_beam_size, selected_beam)
        sen_weights = self._expand_visual(sen_weights, cur_beam_size, selected_beam)
        ocr_list = self._expand_visual(ocr_list, cur_beam_size, selected_beam)
        ocr_vec = self._expand_visual(ocr_vec, cur_beam_size, selected_beam)
        shot_mask = self._expand_visual(shot_mask, cur_beam_size, selected_beam)
        ...
        return visual, object_feats, geo_weights, sen_weights, ocr_list, ocr_vec, shot_mask, outputs
