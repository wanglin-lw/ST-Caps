...
class CaptioningModel(Module):
    ...
    def beam_search(self, visual: utils.TensorOrSequence, object_feats: utils.TensorOrSequence, geo_weights: utils.TensorOrSequence, sen_weights: utils.TensorOrSequence, ocr_list: utils.TensorOrSequence, ocr_vec: utils.TensorOrSequence, shot_mask: utils.TensorOrSequence, max_len: int, eos_idx: int, beam_size: int, out_size=1,
                    return_probs=False, **kwargs):
        bs = BeamSearch(self, max_len, eos_idx, beam_size)
        return bs.apply(visual, object_feats, geo_weights, sen_weights, ocr_list, ocr_vec, shot_mask, out_size, return_probs, **kwargs)
