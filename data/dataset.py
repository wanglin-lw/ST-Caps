...
class TaoCaps(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root):
        roots = {}
        roots['train'] = {
            'img': os.path.join(img_root, 'Tao_resnet152.hdf5'),
            'cap': os.path.join(ann_root, 'train.txt')
        }
        roots['val'] = {
            'img': os.path.join(img_root, 'Tao_resnet152.hdf5'),
            'cap': os.path.join(ann_root, 'val.txt')
        }
        roots['test'] = {
            'img': os.path.join(img_root, 'Tao_resnet152.hdf5'),
            'cap': os.path.join(ann_root, 'test.txt')
        }
        roots['trainrestval'] = {
        ...
        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(roots)
        examples = self.train_examples + self.val_examples + self.test_examples
        super(TaoCaps, self).__init__(examples, {'image': image_field, 'text': text_field})
    ...
    def get_samples(cls, roots):
        ...
        for split in ['val', 'train', 'test']:
            ...
            with open('data/Tao/metadata/' + split + '.txt', 'r') as fin:
                for line in fin:
                    line = line.strip()
                    vid = line.split(',')[0]
                    caption = line.split(',')[1]
                    captions.setdefault(vid, []).append(caption)

            fin = h5py.File(roots[split]['img'], 'r')
            
            for vid in list(fin.keys()):
                vid = vid.split('.')[0]
                for cap_i in captions[vid]:
                    example = Example.fromdict({'image': vid, 'text': cap_i})
                    if split == 'train':
